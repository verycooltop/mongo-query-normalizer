...
describe("invariants / normalize 输出", function () {
    this.timeout(120000);

    it("predicate：输出仍为可 JSON 序列化的 plain 结构（不含循环）", function () {
        const { seed, numRuns } = getFcConfig();
        fc.assert(
            fc.property(queryArbExtended, (q) => {
                const { query, meta } = normalizeQuery(q, { level: "predicate" });
    ...
    it("normalize 不应把逻辑树吹到不合理深度（相对输入）", function () {
        const { seed, numRuns } = getFcConfig();
        fc.assert(
    ...
    it("扩展 queryArb：输出逻辑深度相对输入仍受控（略放宽上界）", function () {
        const { seed, numRuns } = getFcConfig();
        fc.assert(
            fc.property(queryArbExtended, (q) => {
                const inD = maxLogicalDepth(q);
    ...
    it("canonical 重复应用：对已 normalize 的 query 再次 normalize 结构稳定", function () {
        const { seed, numRuns } = getFcConfig();
        fc.assert(
            fc.property(queryArbExtended, (q) => {
    ...
    it("normalize 后 JSON 体积相对输入不应无理由暴涨（结构性防失控）", function () {
        const { seed, numRuns } = getFcConfig();
        fc.assert(
            fc.property(queryArbExtended, (q) => {
